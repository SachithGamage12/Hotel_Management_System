...
try {
    ...
    // Fetch ALL today's bookings (ignore time range)
    $today_query = "
        SELECT wb.booking_reference, wb.full_name, wb.couple_name, wb.no_of_pax, 
               ft.name AS function_name, v.name AS venue_name,
    ...
    // Fetch monthly payment totals (last 24 months)
...
<body>
            ...
            <div class="dashboard">
                ...
                <div class="bookings-container">
                    ...
                    <div class="booking-box">
                        ...
                        <?php else: ?>
                            <p class="no-bookings">No bookings today.</p>
                        <?php endif; ?>
                    </div>
                </div>
    ...
    <script>
        ...
        function updateChartColors() {
            [dailyChart, monthlyChart, yearlyChart].forEach(chart => {
                chart.options.scales.y.title.color = document.body.dataset.theme === 'dark' ? '#ffffff' : '#2d3436';
                chart.options.scales.y.ticks.color = document.body.dataset.theme === 'dark' ? '#ffffff' : '#2d3436';
                chart.options.scales.x.ticks.color = document.body.dataset.theme === 'dark' ? '#ffffff' : '#2d3436';
                chart.options.plugins.legend.labels.color = document.body.dataset.theme === 'dark' ? '#ffffff' : '#2d3436';
                chart.options.scales.y.grid.color = document.body.dataset.theme === 'dark' ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0, 0, 0, 0.1)';
                chart.options.plugins.tooltip.backgroundColor = document.body.dataset.theme === 'dark' ? 'rgba(0, 0, 0, 0.8)' : 'rgba(255, 255, 255, 0.8)';
                chart.options.plugins.tooltip.titleColor = document.body.dataset.theme === 'dark' ? '#ffffff' : '#2d3436';
                chart.options.plugins.tooltip.bodyColor = document.body.dataset.theme === 'dark' ? '#ffffff' : '#2d3436';
                chart.options.plugins.tooltip.borderColor = document.body.dataset.theme === 'dark' ? '#ffffff' : '#2d3436';
                chart.update();
            });
        }
    </script>
