...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'add_booking') {
    ...
    try {
        ...
        $insertStmt = $conn->prepare("
            INSERT INTO room_bookings (guest_name, telephone, check_in, check_out, room_number, pax, remarks, function_type, booking_type, created_at)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'edit_booking') {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT room_number FROM room_bookings WHERE id = :booking_id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'postpone_booking') {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT room_number, guest_name, telephone, pax, remarks, function_type FROM room_bookings WHERE id = :booking_id");
        $stmt->execute(['booking_id' => $booking_id]);
        $booking = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        $roomStmt = $conn->prepare("SELECT COUNT(*) FROM rooms WHERE room_number = :room_number");
...
if ($action === 'get_bookings') {
    ...
    try {
        // Ensure the dates include time components for proper comparison
