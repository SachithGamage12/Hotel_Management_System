...
if (isset($_POST['order_location']) && !isset($_POST['add_item']) && !isset($_POST['remove_item'])) {
    ...
    if (isset($_POST['grn_datetime']) && !empty($_POST['grn_datetime'])) {
        try {
            $selected_datetime = new DateTime($_POST['grn_datetime'], new DateTimeZone('Asia/Kolkata'));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['item_name'], $_POST['quantity'], $_POST['unit'], $_POST['add_item'])) {
    ...
    error_log("Received item data: item_name=$item_name, quantity=$quantity_input, unit=$unit");
    ...
    if (!is_numeric($quantity_input) || floatval($quantity_input) <= 0) {
        ...
        if ($stmt === false) {
            ...
            if ($result->num_rows > 0) {
                ...
                error_log("Item validation failed: item_name=$item_name, unit=$unit");
            }
            $stmt->close();
        }
    }
...
if (isset($_POST['save_grn']) && !empty($_SESSION['grn_items']) && !empty($_SESSION['grn_number'])) {
    ...
    $date = (new DateTime($_SESSION['grn_datetime'], new DateTimeZone('Asia/Kolkata')))->format("Y-m-d H:i:s");
                ...
                if ($item_stmt === false) {
                    ...
                    foreach ($_SESSION['grn_items'] as $item) {
                        ...
                        if ($stock_result->num_rows > 0) {
                            ...
                            $update_stmt->bind_param("ddsisd", $new_available, $quantity, $added_date, $new_total, $item_id, $location);
                            ...
                            $insert_stmt->bind_param("isdisd", $item_id, $location, $new_available, $quantity, $added_date, $new_total);
...
<head>
    ...
    <style>
        * {
        ...
        select:focus, input[type="text"]:focus, input[type="password"]:focus, input[type="datetime-local"]:focus {
            border-color: #000;
            outline: none;
        }
        .alert {
        ...
        @media print {
            ...
            .controls, .form-container, .user-info, .alert, .responsible-form, .remove-btn, .back-button {
                display: none !important;
            }
        }
    </style>
...
<body>
    <div class="wrapper">
        <button onclick="window.location.href='../stores.php'" class="back-button no-print">
            Back
        </button>
        ...
        <div class="no-print">
            ...
            <div class="form-container">
                <form method="POST" action="">
                    ...
                    <div class="form-group">
                        <label for="grn_datetime">GRN Date and Time (IST)</label>
            ...
            <div class="form-container">
                <form method="POST" action="" onsubmit="return validateForm()" id="add-item-form">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="quantity" name="quantity" placeholder="Enter quantity (e.g., 1.5, 2.25)" pattern="[0-9]*\.?[0-9]+" title="Please enter a valid number (decimal allowed)" required>
                    </div>
        ...
        <div class="grn-container">
            <div class="grn-header">
                ...
                <div><strong>Date</strong><span class="colon">: </span><span id="print_date"><?php echo (new DateTime($_SESSION['grn_datetime'], new DateTimeZone('Asia/Kolkata')))->format('d-M-Y H:i'); ?></span></div>
                        ...
                        <?php foreach ($_SESSION['grn_items'] as $index => $item): ?>
                            <tr>
                                ...
                                <td><?php 
                                    ...
                                    echo (floor($quantity) == $quantity) ? (int)$quantity : number_format($quantity, 2, '.', '');
                                ?></td>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $("#grn-form").on('submit', function(e) {
            if (isPrinting) {
                console.warn("Form submission blocked: Print operation in progress.");
                e.preventDefault();
                return false;
            }
        });
    ...
    function validateForm() {
        ...
        if (!quantity || !isValidDecimal(quantity)) {
            alert('Please enter a valid positive number (decimal allowed).');
            return false;
        }
        return true;
    }
    ...
    function printGRN() {
        if (isPrinting) {
            console.warn("Print operation already in progress, aborting.");
            return;
        }
        isPrinting = true;
        ...
        // Remove the Action column from cloned table
        const rows = tableClone.getElementsByTagName('tr');
        for (let i = 0; i < rows.length; i++) {
            const cells = rows[i].getElementsByTagName('td');
            if (cells.length === 4) {
                cells[3].remove(); // Remove the "Action" column
            }
        }
        const itemsHtml = tableClone.innerHTML;

        // Check if GRN has already been printed
        if (sessionStorage.getItem('printed_grn_' + grnNo)) {
            console.error("GRN already printed:", grnNo);
            alert("Error: This GRN has already been printed.");
            isPrinting = false;
        ...
        const printWindow = window.open('', '_blank', 'width=80mm,height=297mm,top=100,left=100');
        if (!printWindow) {
        ...
        // Mark as printed and handle cleanup
        sessionStorage.setItem('printed_grn_' + grnNo, 'true');

        // Listen for the print window to close
        const checkPrintWindow = setInterval(() => {
            if (printWindow.closed) {
                clearInterval(checkPrintWindow);
                console.log("Print window closed, submitting form.");
                document.getElementById('grn-form').submit();
                setTimeout(() => {
                    console.log("Redirecting to current page");
        ...
        setTimeout(() => {
            if (isPrinting) {
                clearInterval(checkPrintWindow);
                console.log("Fallback: Submitting form after timeout.");
                document.getElementById('grn-form').submit();
                setTimeout(() => {
                    console.log("Fallback: Redirecting to current page");
                    window.location.href = '<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>';
                    isPrinting = false;
                    $('#print-btn').prop('disabled', false);
                }, 1000);
