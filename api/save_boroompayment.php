...
try {
    ...
    // Validate added_amount when amount_type is 'Added to package', 'Add to Menu', or 'Add to Repayment'
    if (in_array($data['amount_type'], ['Added to package', 'Add to Menu', 'Add to Repayment'])) {
        if (!isset($data['added_amount']) || empty(trim($data['added_amount'])) || !is_numeric($data['added_amount']) || (float)$data['added_amount'] < 0) {
            throw new Exception('Added amount is required and must be a non-negative number when amount type is ' . $data['amount_type']);
        }
    }
    ...
    $total_amount = $data['amount_type'] === 'FOC' || in_array($data['amount_type'], ['Added to package', 'Add to Menu', 'Add to Repayment']) ? null : (isset($data['total_amount']) && trim($data['total_amount']) !== '' ? (float)$data['total_amount'] : null);
    $advance_payment = $data['amount_type'] === 'FOC' || in_array($data['amount_type'], ['Added to package', 'Add to Menu', 'Add to Repayment']) ? null : (isset($data['advance_payment']) && trim($data['advance_payment']) !== '' ? (float)$data['advance_payment'] : null);
    $pending_amount = $data['amount_type'] === 'FOC' || in_array($data['amount_type'], ['Added to package', 'Add to Menu', 'Add to Repayment']) ? null : (isset($data['pending_amount']) && trim($data['pending_amount']) !== '' ? (float)$data['pending_amount'] : null);
    $added_amount = in_array($data['amount_type'], ['Added to package', 'Add to Menu', 'Add to Repayment']) ? (isset($data['added_amount']) && trim($data['added_amount']) !== '' ? (float)$data['added_amount'] : null) : null;
