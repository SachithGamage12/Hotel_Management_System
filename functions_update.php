...
if (isset($_GET['action']) && in_array($_GET['action'], ['mark_paid', 'mark_credit', 'mark_refunded']) && !empty($_GET['id'])) {
    ...
    try {
        ...
        // Get current row
        $stmt = $pdo->prepare("SELECT total_amount, payment_amount, pending_amount, payment_status FROM payments WHERE id = ?");
        $stmt->execute([$id]);
        ...
        // If marking Paid and pending > 0 → update payment_amount to cover full amount
        if ($newStatus === 'Paid' && ($row['pending_amount'] ?? 0) > 0) {
            $newPaymentAmount = $row['total_amount'];
            $newPendingAmount = 0;
            
            $update = $pdo->prepare("UPDATE payments SET payment_amount = ?, pending_amount = ?, payment_status = 'Paid' WHERE id = ?");
            $update->execute([$newPaymentAmount, $newPendingAmount, $id]);
        } else {
            // Just update status
            $update = $pdo->prepare("UPDATE payments SET payment_status = ? WHERE id = ?");
            $update->execute([$newStatus, $id]);
            $newPaymentAmount = $row['payment_amount'];
            $newPendingAmount = $row['pending_amount'];
        }
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    if (!empty($_GET['search'])) {
        ...
        $whereConditions[] = "(booking_reference LIKE :search OR invoice_number LIKE :search OR issued_by LIKE :search OR contact_no LIKE :search OR email LIKE :search)";
        $params[':search'] = $searchTerm;
    }
    
    if (!empty($_GET['value_type'])) {
        $whereConditions[] = "value_type = :value_type";
        $params[':value_type'] = $_GET['value_type'];
    }
    ...
    $exportQuery = "
        SELECT 
            id, booking_reference, invoice_number, value_type, 
            total_amount, payment_amount, pending_amount,
            no_of_pax, day_or_night, rate_per_plate, additional_plate_rate,
            issued_by, contact_no, whatsapp_no, email, payment_date, remarks, payment_status
        FROM payments 
        $whereClause 
    ...
    fputcsv($output, ['ID','Ref','Invoice','Type','Total','Paid','Pending','Pax','Day/Night','Rate/Plate','Addl Plate Rate','Issued','Contact','WhatsApp','Email','Date','Remarks','Status']);
    foreach ($data as $row) {
        fputcsv($output, [
            $row['id'],
...
if (!empty($_GET['search'])) {
    $searchTerm = '%' . $_GET['search'] . '%';
    $whereConditions[] = "(booking_reference LIKE :search OR invoice_number LIKE :search OR issued_by LIKE :search OR contact_no LIKE :search OR email LIKE :search OR remarks LIKE :search)";
    $params[':search'] = $searchTerm;
}
...
$allowedSort = ['id','booking_reference','invoice_number','value_type','total_amount','payment_amount','pending_amount','payment_date','payment_status'];
...
$statsStmt = $pdo->prepare("
    SELECT 
        ...
        -- Payment Amounts by Payment Status (EXCLUDE CANCELLED)
        SUM(CASE WHEN payment_status = 'Paid' THEN COALESCE(payment_amount, 0) ELSE 0 END) as payment_paid,
        SUM(CASE WHEN payment_status = 'Pending' THEN COALESCE(payment_amount, 0) ELSE 0 END) as payment_pending,
        SUM(CASE WHEN payment_status = 'Partial' THEN COALESCE(payment_amount, 0) ELSE 0 END) as payment_partial,
        SUM(CASE WHEN payment_status = 'Credit' THEN COALESCE(payment_amount, 0) ELSE 0 END) as payment_credit,
        SUM(CASE WHEN payment_status = 'Refunded' THEN COALESCE(payment_amount, 0) ELSE 0 END) as payment_refunded,
        
        -- Pending Balance ONLY from Pending & Partial status (EXCLUDE CANCELLED)
        SUM(CASE 
            WHEN payment_status IN ('Pending', 'Partial') 
            THEN GREATEST(COALESCE(pending_amount, 0), 0)
            ELSE 0 
        ...
        SUM(COALESCE(payment_amount, 0)) as total_payment_collected
...
$netCollection = ($stats['payment_paid'] ?? 0) + ($stats['payment_partial'] ?? 0) - ($stats['payment_refunded'] ?? 0);

// Get cancelled count separately
$cancelStmt = $pdo->prepare("SELECT COUNT(*) as cancel_count FROM payments WHERE payment_status = 'Cancel'");
$cancelStmt->execute();
$cancelCount = $cancelStmt->fetchColumn();

// Value Types
$valueTypes = $pdo->query("SELECT DISTINCT value_type FROM payments WHERE value_type IS NOT NULL AND value_type != '' ORDER BY value_type")->fetchAll(PDO::FETCH_COLUMN);
...
<head>
    ...
    <title>Payments Report - Payment Status Based</title>
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        body { font-family:Segoe UI,Tahoma,sans-serif; background:#667eea; padding:20px; }
        .container { max-width:1800px; margin:auto; background:rgba(255,255,255,.95); border-radius:20px; box-shadow:0 20px 40px rgba(0,0,0,.1); overflow:hidden; }
...
<body>
    ...
    <div class="container">
        <div class="header">
            <h1>Payments Report - Payment Status Based</h1>
        ...
        <div class="controls">
            <form method="GET">
                <div class="controls-row">
                    <div class="control-group">
                        <label>Search</label>
                        <input type="text" name="search" value="<?php echo htmlspecialchars($_GET['search'] ?? ''); ?>" placeholder="Ref, invoice, name, email...">
                    </div>
                    <div class="control-group">
                        <label>Value Type</label>
                        <select name="value_type">
                            <option value="">All</option>
                            <?php foreach ($valueTypes as $t): ?>
                                <option value="<?php echo htmlspecialchars($t); ?>" <?php echo ($_GET['value_type'] ?? '') == $t ? 'selected' : ''; ?>>
            ...
            <div class="summary-grid">
                ...
                <div class="summary-card">
                    <h3>Payment Collection by Status (Active)</h3>
                    <div class="summary-item">
                        <span>Paid Invoices:</span>
                        <span><?php echo fmt($stats['payment_paid'] ?? 0); ?></span>
                    </div>
                    <div class="summary-item">
                        <span>Pending Invoices:</span>
                        <span><?php echo fmt($stats['payment_pending'] ?? 0); ?></span>
                    </div>
                    <div class="summary-item">
                        <span>Partial Invoices:</span>
                        <span><?php echo fmt($stats['payment_partial'] ?? 0); ?></span>
                    </div>
                    <div class="summary-item">
                        <span>Credit Invoices:</span>
                        <span><?php echo fmt($stats['payment_credit'] ?? 0); ?></span>
                    </div>
                    <div class="summary-item">
                        <span>Refunded Invoices:</span>
                        <span><?php echo fmt($stats['payment_refunded'] ?? 0); ?></span>
                    </div>
                    <div class="summary-item total">
                        <span>Total Payment (Active):</span>
                        <span><?php echo fmt($stats['total_payment_collected'] ?? 0); ?></span>
                    </div>
                </div>
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr data-id="<?php echo $p['id']; ?>">
                                ...
                                <td><?php echo htmlspecialchars($p['value_type'] ?: '-'); ?></td>
                                <td class="amount"><?php echo fmt($p['total_amount']); ?></td>
                                <td class="amount payment" data-payment="<?php echo $p['payment_amount']; ?>">
                                    <?php echo fmt($p['payment_amount']); ?>
                                </td>
                                <td class="amount pending <?php echo $pending > 0 ? 'negative' : ''; ?>" data-pending="<?php echo $pending; ?>">
                                    <?php echo fmt($pending); ?>
                                </td>
                                <td><?php echo $p['no_of_pax'] ?: '-'; ?></td>
                                <td><?php echo htmlspecialchars($p['day_or_night'] ?: '-'); ?></td>
                                <td class="amount"><?php echo $p['rate_per_plate'] ? fmt($p['rate_per_plate']) : '-'; ?></td>
                                <td>
                                ...
                                <td><?php echo htmlspecialchars($p['whatsapp_no'] ?: '-'); ?></td>
