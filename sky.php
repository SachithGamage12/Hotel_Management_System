...
try {
    ...
    $pending_query = $conn->query("SELECT COUNT(DISTINCT order_sheet_no) as count FROM order_sheet WHERE status = 'pending'");
    $pending_count = $pending_query->fetch(PDO::FETCH_ASSOC)['count'];
    
    $issued_query = $conn->query("SELECT COUNT(DISTINCT order_sheet_no) as count FROM order_sheet WHERE status = 'issued'");
...
<head>
    ...
    <title>Sky Restaurant Panel</title>
...
<body>
    <div class="admin-panel">
        <div class="sidebar">
            ...
            <div class="logo">
                <img src="images/sky logo.png" alt="Grand Guardian Logo" class="grand-guardian-logo">
            </div>
            <div class="menu">
                ...
                <div class="menu-item" onclick="window.location.href='sky_kitchen/order_sheet.php'">
                    <i class="fas fa-shopping-cart"></i>
                    <span>Order Sheet</span>
                </div>
                <div class="menu-item" onclick="window.location.href='sky_kitchen/skykitchen_usage.php'">
                    <i class="fas fa-line-chart"></i>
                    <span>Buffer Stock Update</span>
                </div>
                <div class="menu-item" onclick="window.location.href='sky_logout.php'">
            ...
            <div class="dashboard">
                ...
                <div class="chart-container">
                    ...
                    <div class="chart-tabs">
                        <div class="chart-tab active" data-chart="daily">Daily</div>
                        <div class="chart-tab" data-chart="monthly">Monthly</div>
                        <div class="chart-tab" data-chart="yearly">Yearly</div>
                    </div>
    ...
    <script>
        ...
        const colors = [
            '#6c5ce7', '#e84393', '#00b894', '#0984e3', 
            '#fdcb6e', '#d63031', '#2d3436', '#636e72'
        ];
        
        function prepareChartData(data, periodKey) {
            const periods = [...new Set(data.map(item => item.period))].sort();
            const functionTypes = [...new Set(data.map(item => item.function_type))];
            
            return {
                labels: periods,
                datasets: functionTypes.map((type, index) => ({
                    label: type,
                    data: periods.map(period => {
                        const item = data.find(d => d.period === period && d.function_type === type);
        ...
        createChart(prepareChartData(dailyData, 'period'));
        ...
        document.querySelectorAll('.chart-tab').forEach(tab => {
            tab.addEventListener('click', function() {
                ...
                if (chartType === 'daily') {
                    ...
                    createChart(prepareChartData(monthlyData, 'period'));
                } else {
                    createChart(prepareChartData(yearlyData, 'period'));
                }
            });
        });
        ...
        themeToggle.addEventListener('click', function() {
            const chartType = document.querySelector('.chart-tab.active').dataset.chart;
            if (chartType === 'daily') {
                createChart(prepareChartData(dailyData, 'period'));
            } else if (chartType === 'monthly') {
                createChart(prepareChartData(monthlyData, 'period'));
        ...
        // Auto-logout after 5 minutes of no mouse clicks or key presses
        let inactivityTimeout;

        function resetInactivityTimer() {
            clearTimeout(inactivityTimeout); // Clear the previous timeout
            inactivityTimeout = setTimeout(() => {
                window.location.href = 'sky_logout.php'; // Redirect to logout page
            }, 300000); // 5 minutes = 300,000 milliseconds
        }

        // Events to monitor for user activity (only clicks and key presses)
