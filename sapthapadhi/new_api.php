...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'add_booking') {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT COUNT(*) FROM sapthapadhirooms WHERE room_number = :room_number");
        $overlapStmt = $conn->prepare("
            SELECT COUNT(*) FROM sapthapadhiroom_bookings
        ...
        $insertStmt = $conn->prepare("
            INSERT INTO sapthapadhiroom_bookings (guest_name, telephone, check_in, check_out, room_number, pax, remarks, function_type, created_at)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'delete_booking') {
    ...
    try {
        $stmt = $conn->prepare("DELETE FROM sapthapadhiroom_bookings WHERE id = :booking_id");
...
if ($action === 'get_rooms') {
    try {
        $stmt = $conn->prepare("SELECT room_number FROM sapthapadhirooms ORDER BY room_number");
        $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'add_room') {
    ...
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO sapthapadhirooms (room_number) VALUES (:room_number)");
        $stmt->execute(['room_number' => $room_number]);
        echo json_encode(['success' => true]);
    } catch (PDOException $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'delete_room') {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT COUNT(*) FROM sapthapadhiroom_bookings WHERE room_number = :room_number");
        $stmt->execute(['room_number' => $room_number]);
        if ($stmt->fetchColumn() > 0) {
        ...
        $stmt = $conn->prepare("DELETE FROM sapthapadhirooms WHERE room_number = :room_number");
        $stmt->execute(['room_number' => $room_number]);
        echo json_encode(['success' => true]);
    } catch (PDOException $e) {
