...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_invoice') {
    try {
        ...
        $stmt = $conn->query("SELECT last_invoice_number FROM sapthapadhiroom_invoice_counter ORDER BY id DESC LIMIT 1");
        $last_invoice = $stmt->fetchColumn();
        $new_invoice_number = 'INV-' . ($last_invoice + 1);

        // Update invoice counter
        $stmt = $conn->prepare("INSERT INTO sapthapadhiroom_invoice_counter (last_invoice_number) VALUES (:last_invoice_number)");
        ...
        $stmt = $conn->prepare("
            INSERT INTO sapthapadhiroom_payments (
