...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['bill_number'])) {
    try {
        ...
        // Fetch invoice details including discount, additional_hours, and hourly_rate
        $stmt = $conn->prepare("
            SELECT rp.*, g.*, mp.name AS meal_plan_name, ric.last_invoice_number
            FROM sapthapadhiroom_payments rp
            LEFT JOIN sapthapadhiguests g ON rp.booking_reference = g.grc_number
            LEFT JOIN meal_plans mp ON g.meal_plan_id = mp.id
            LEFT JOIN sapthapadhiroom_invoice_counter ric ON rp.invoice_number = CONCAT('INV-', ric.last_invoice_number)
...
<body>
    ...
    <div class="receipt-container">
        ...
        <div class="footer<?php echo $guest ? ' active' : ''; ?>">
            ...
            <div class="thank-you">Thank you for choosing Hotel Sapthapadhi</div>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.onafterprint = function() {
            ...
            hoursCountDisplay.textContent = '0';
            additionalHourChargeDisplay.textContent = 'Rs. 0.00';
            subtotalDisplay.textContent = '<?php echo is_numeric($total_rate) ? 'Rs. ' . number_format($total_rate, 2) : 'N/A'; ?>';
            discountDisplay.textContent = 'Rs. 0.00';
            finalTotalDisplay.textContent = '<?php echo is_numeric($final_total) ? 'Rs. ' . number_format($final_total, 2) : 'N/A'; ?>';
            advancePaymentDisplay.textContent = 'Rs. 0.00';
            pendingAmountDisplay.textContent = '<?php echo is_numeric($pending_amount) ? 'Rs. ' . number_format($pending_amount, 2) : 'N/A'; ?>';
