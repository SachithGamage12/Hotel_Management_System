...
<body class="min-h-screen flex flex-col">
  ...
  <div class="w-full min-h-screen flex flex-col">
    ...
    <div class="header flex justify-between items-center">
      <img src="../image/sapthapadhi logo png.png" alt="Hotel Logo" class="h-14 rounded-full animate-pulseGlow">
    ...
    <div class="flex flex-1 items-center justify-center p-8">
      <div class="form-container w-full max-w-5xl p-10 flex flex-col gap-10 animate-fadeInUp">
        ...
        <div id="page2" class="form-page hidden">
          ...
          <form id="services-form" action="save_guest.php" method="POST" class="grid grid-cols-2 gap-8">
  ...
  <script>
    ...
    async function fetchNextGRCNumber() {
      try {
        const response = await fetch('../sapi/fetch_next_grc.php');
        if (!response.ok) {
        ...
        if (result.success) {
          ...
          document.getElementById('grc_number').value = '';
          alert('Unable to fetch GRC number: ' + result.message);
          disableFormSubmission();
        }
      } catch (error) {
        console.error('Error fetching GRC number:', error);
        document.getElementById('grc_number').value = '';
        alert('An error occurred while fetching the GRC number. Please try again.');
        disableFormSubmission();
      }
    }
    ...
    async function fetchRoomRates() {
      ...
      try {
        const response = await fetch(`../sapi/fetch_room_rate.php?room_data=${encodeURIComponent(roomQuery)}`, {
          method: 'GET',
    ...
    async function populateRoomTypes(targetSelects = null) {
      try {
        const response = await fetch('../sapi/fetch_options.php?field_type=room_types');
    ...
    async function populateSelect(selectId, fieldType) {
      try {
        const response = await fetch(`../sapi/fetch_options.php?field_type=${fieldType}`);
        if (!response.ok) throw new Error(`Network response was not ok: ${response.status}`);
        const items = await response.json();
    ...
    document.getElementById('add-field-form').addEventListener('submit', async (e) => {
      ...
      try {
        const response = await fetch('../sapi/create_option.php', {
          method: 'POST',
          body: formData
        });
        const result = await response.json();
    ...
    document.getElementById('services-form').addEventListener('submit', async (e) => {
      ...
      const grcNumber = document.getElementById('grc_number').value;
      if (!grcNumber || isNaN(grcNumber) || parseInt(grcNumber) <= 0) {
        alert('Invalid GRC number. Please ensure a valid GRC number is fetched before submitting.');
        return;
      }
      ...
      try {
        const response = await fetch('../sapi/save_guest.php', {
          method: 'POST',
          body: formData
        });
        const result = await response.json();
