...
$invoices = [];
$error_message = '';
$summary = ['total_invoices' => 0, 'total_amount' => 0, 'customer_count' => 0, 'staff_count' => 0];
...
try {
    ...
    // Payment type filter
    if ($payment_filter === 'customer') {
        $query .= " AND payment_type = 'cash_customer'";
    } elseif ($payment_filter === 'staff') {
        $query .= " AND payment_type = 'cash_staff'";
    } else {
        $query .= " AND payment_type IN ('cash_customer', 'cash_staff')";
    }
    ...
    foreach ($invoices as $invoice) {
        ...
        if ($invoice['payment_type'] === 'cash_customer') {
...
<head>
    ...
    <title>Guardia POS - Cash Invoices</title>
    <style>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="summary-grid">
            ...
            <div class="summary-card">
                <div class="summary-label">Customer Invoices</div>
                <div class="summary-value" style="color: var(--green);"><?php echo $summary['customer_count']; ?></div>
            </div>
            <div class="summary-card">
                <div class="summary-label">Staff Invoices</div>
                <div class="summary-value" style="color: var(--blue);"><?php echo $summary['staff_count']; ?></div>
            </div>
        </div>
            ...
            <form class="filter-form" id="filterForm">
                <div class="form-group">
                    ...
                    <select name="payment_filter" id="paymentFilter">
                        <option value="all" <?php echo $payment_filter === 'all' ? 'selected' : ''; ?>>All Cash</option>
                        <option value="customer" <?php echo $payment_filter === 'customer' ? 'selected' : ''; ?>>Cash Customer</option>
                        <option value="staff" <?php echo $payment_filter === 'staff' ? 'selected' : ''; ?>>Cash Staff</option>
                    </select>
                </div>
                <div class="form-group">
        ...
        <!-- Invoices Table -->
        <div class="card">
            <h2 class="card-title">Cash Invoices - <?php 
                $display_filter = ucfirst($filter_type);
                if ($payment_filter !== 'all') {
                    $display_filter .= ' | ' . ucfirst($payment_filter);
                }
                echo $display_filter . ': ' . $filter_value; 
            ?></h2>
            <?php if (empty($invoices)): ?>
                <div class="no-data">
                    <p>No cash invoices found for the selected criteria.</p>
                </div>
            <?php else: ?>
                        ...
                        <tbody>
                            <?php foreach ($invoices as $invoice): ?>
                                <tr data-invoice-id="<?php echo htmlspecialchars((int)$invoice['id']); ?>">
                                    ...
                                    <td>
                                        <span class="badge <?php echo $invoice['payment_type'] === 'cash_customer' ? 'badge-customer' : 'badge-staff'; ?>">
                                            <?php echo htmlspecialchars($invoice['payment_type'] === 'cash_customer' ? 'Customer' : 'Staff'); ?>
                                        </span>
                                    </td>
...
document.getElementById('filterForm').addEventListener('submit', function(e) {
    ...
    window.location.href = `cash.php?filter_type=${filterType}&filter_value=${encodeURIComponent(filterValue)}&payment_filter=${paymentFilter}`;
});
