...
try {
    ...
    // Build base query
    $query = "
        SELECT id, invoice_number, created_at, payment_type, creditor_name, other_creditor_name, cashier,
    ...
    if ($payment_filter === 'credit') {
        $query .= " AND payment_type = 'credit'";
    } elseif ($payment_filter === 'other_credit') {
        $query .= " AND payment_type = 'other_credit'";
    } else {
        $query .= " AND payment_type IN ('credit', 'other_credit')";
    }
...
<head>
    ...
    <title>Guardia POS - Credit Invoices</title>
    <style>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="summary-grid">
            ...
            <div class="summary-card">
                <div class="summary-label">Credit Invoices</div>
                <div class="summary-value" style="color: var(--pink);"><?php echo $summary['credit_count']; ?></div>
            </div>
            <div class="summary-card">
                <div class="summary-label">Other Credit Invoices</div>
                <div class="summary-value" style="color: var(--orange);"><?php echo $summary['other_credit_count']; ?></div>
            </div>
        </div>
            ...
            <form class="filter-form" id="filterForm">
                <div class="form-group">
                    ...
                    <select name="payment_filter" id="paymentFilter">
                        <option value="all" <?php echo $payment_filter === 'all' ? 'selected' : ''; ?>>All Credit</option>
                        <option value="credit" <?php echo $payment_filter === 'credit' ? 'selected' : ''; ?>>Credit</option>
                        <option value="other_credit" <?php echo $payment_filter === 'other_credit' ? 'selected' : ''; ?>>Other Credit</option>
                    </select>
                </div>
                <div class="form-group">
        ...
        <!-- Invoices Table -->
        <div class="card">
            <h2 class="card-title">Credit Invoices - <?php 
                $display_filter = ucfirst($filter_type);
                if ($payment_filter !== 'all') {
                    $display_filter .= ' | ' . ($payment_filter === 'credit' ? 'Credit' : 'Other Credit');
                }
                echo $display_filter . ': ' . htmlspecialchars($filter_value); 
            ?></h2>
            <?php if (empty($invoices)): ?>
                <div class="no-data">
                    <p>No credit invoices found for the selected criteria.</p>
                </div>
            <?php else: ?>
                        ...
                        <tbody>
                            <?php foreach ($invoices as $invoice): ?>
                                <tr data-invoice-id="<?php echo htmlspecialchars((int)$invoice['id']); ?>">
                                    ...
                                    <td>
                                        <span class="badge <?php echo $invoice['payment_type'] === 'credit' ? 'badge-credit' : 'badge-other-credit'; ?>">
                                            <?php echo htmlspecialchars($invoice['payment_type'] === 'credit' ? 'Credit' : 'Other Credit'); ?>
                                        </span>
                                    </td>
                                    <td><?php echo htmlspecialchars($invoice['payment_type'] === 'credit' ? ($invoice['creditor_name'] ?? 'N/A') : ($invoice['other_creditor_name'] ?? 'N/A')); ?></td>
    ...
    <script>
        ...
        document.getElementById('filterForm').addEventListener('submit', function(e) {
            ...
            window.location.href = `credit.php?filter_type=${filterType}&filter_value=${encodeURIComponent(filterValue)}&payment_filter=${paymentFilter}`;
        });
