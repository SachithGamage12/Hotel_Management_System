...
try {
    ...
    // Build base query
    $query = "
        SELECT id, invoice_number, created_at, payment_type, foc_responsible, cashier,
               subtotal, discount, service_charge, COALESCE(delivery_charge, 0) AS delivery_charge, grand_total
        FROM invoices
        WHERE status != 'canceled' AND payment_type = 'foc'
    ";
...
<head>
    ...
    <title>Guardia POS - FOC Invoices</title>
    <style>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="summary-grid">
            <div class="summary-card">
                <div class="summary-label">Total FOC Invoices</div>
        ...
        <!-- Invoices Table -->
        <div class="card">
            <h2 class="card-title">FOC Invoices - <?php 
                $display_filter = ucfirst($filter_type) . ': ' . htmlspecialchars($filter_value); 
                echo $display_filter; 
            ?></h2>
            <?php if (empty($invoices)): ?>
                <div class="no-data">
                    <p>No FOC invoices found for the selected criteria.</p>
                </div>
            <?php else: ?>
                        ...
                        <tbody>
                            <?php foreach ($invoices as $invoice): ?>
                                <tr data-invoice-id="<?php echo htmlspecialchars((int)$invoice['id']); ?>">
                                    ...
                                    <td>
                                        <span class="badge badge-foc">FOC</span>
                                    </td>
                                    <td><?php echo htmlspecialchars($invoice['foc_responsible'] ?? 'N/A'); ?></td>
    ...
    <script>
        ...
        document.getElementById('filterForm').addEventListener('submit', function(e) {
            ...
            window.location.href = `foc.php?filter_type=${filterType}&filter_value=${encodeURIComponent(filterValue)}`;
        });
