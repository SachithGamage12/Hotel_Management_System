...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_invoice') {
    ...
    try {
        ...
        $stmt = $conn->query("SELECT COUNT(*) FROM room_invoice_counter WHERE id = 1");
        if ($stmt->fetchColumn() == 0) {
            $conn->exec("INSERT INTO room_invoice_counter (id, last_invoice_number) VALUES (1, 2080)");
        }
        // Get and lock the last invoice number
        $stmt = $conn->query("SELECT last_invoice_number FROM room_invoice_counter WHERE id = 1 LIMIT 1 FOR UPDATE");
        ...
        $stmt = $conn->prepare("UPDATE room_invoice_counter SET last_invoice_number = ? WHERE id = 1");
