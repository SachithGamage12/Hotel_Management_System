...
try {
    ...
    // Fetch data for existing charts (monthly, daily, yearly)
    ...
    // Fetch monthly sales data for rooms and functions
    $room_sales_query = $conn->query("
        ...
        ORDER BY period DESC LIMIT 12
    ");
    $room_sales_data = $room_sales_query->fetchAll(PDO::FETCH_ASSOC);

    $function_sales_query = $conn->query("
        SELECT DATE_FORMAT(payment_date, '%Y-%m') as period, 
               SUM(total_amount) as total_sales
        FROM payments 
        GROUP BY period 
        ORDER BY period DESC LIMIT 12
    ");
    $function_sales_data = $function_sales_query->fetchAll(PDO::FETCH_ASSOC);
    ...
    // Merge periods from both datasets
    $all_periods = array_unique(array_merge(
        array_column($room_sales_data, 'period'),
        array_column($function_sales_data, 'period')
    ));
    rsort($all_periods); // Sort in descending order
    $all_periods = array_slice($all_periods, 0, 12); // Limit to last 12 months

    foreach ($all_periods as $period) {
        $periods[] = $period;
        // Room sales for the period
        $room_sale = array_filter($room_sales_data, fn($row) => $row['period'] === $period);
        $room_sales[] = !empty($room_sale) ? (float)reset($room_sale)['total_sales'] : 0;
        // Function sales for the period
        $function_sale = array_filter($function_sales_data, fn($row) => $row['period'] === $period);
        $function_sales[] = !empty($function_sale) ? (float)reset($function_sale)['total_sales'] : 0;
    }

    // Reverse arrays to display from oldest to newest
    $periods = array_reverse($periods);
    $room_sales = array_reverse($room_sales);
    $function_sales = array_reverse($function_sales);
...
<head>
    ...
    <title>Hgg Accounts Panel</title>
...
<body>
    <div class="admin-panel">
        <div class="sidebar">
            ...
            <div class="menu">
                ...
                <div class="menu-item" onclick="window.location.href='accounts/room_report.php'">
                    <i class="fas fa-bed"></i>
                    <span>HGG Room Sale</span>
                </div>
                <div class="menu-item" onclick="window.location.href='accounts/functions_report.php'">
                    <i class="fas fa-university"></i>
                    <span>HGG Function Sale</span>
                </div>
                <div class="menu-item" onclick="window.location.href='logistic/logestic_stock_report.php'">
                    <i class="fas fa-database"></i>
                    <span>Logistic Stock</span>
                </div>
                <div class="menu-item" onclick="window.location.href='accounts/Main_kitchen_buffer.php'">
                    <i class="fas fa-database"></i>
                    <span>Main/K Stock</span>
                </div>
                <div class="menu-item" onclick="window.location.href='accounts/Hgg_buffer.php'">
                    <i class="fas fa-database"></i>
                    <span>HGG/R Stock</span>
                </div>
                <div class="menu-item" onclick="window.location.href='accounts/Sky_buffer.php'">
                    <i class="fas fa-database"></i>
                    <span>Sky/R Stock</span>
                </div>
                <div class="menu-item" onclick="window.location.href='accounts_logout.php'">
            ...
            <div class="dashboard">
                ...
                <div class="chart-container">
                    ...
                    <div class="chart-tabs">
                        <div class="chart-tab active" data-tab="monthly">Monthly</div>
                    </div>
    ...
    <script>
        ...
        const salesChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [
                    {
                        label: 'Room Sales',
                        data: <?php echo json_encode($room_sales); ?>,
                    ...
                    {
                        label: 'Function Sales',
                        data: <?php echo json_encode($function_sales); ?>,
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                return `${context.dataset.label}: Rs. ${context.parsed.y.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`;
                            }
                        }
                    }
                },
        ...
        // Chart tab switching (for future extensibility)
        document.querySelectorAll('.chart-tab').forEach(tab => {
            tab.addEventListener('click', function() {
                document.querySelectorAll('.chart-tab').forEach(t => t.classList.remove('active'));
                this.classList.add('active');
                // Add logic for other time periods (e.g., yearly) if needed
            });
        });
    </script>
</body>
