...
<body class="min-h-screen flex flex-col">
  ...
  <script>
    ...
    async function fetchNextGRCNumber() {
      try {
        const response = await fetch('api/fetch_next_grc.php');
    ...
    async function fetchRoomRates() {
      ...
      try {
        const response = await fetch(`api/fetch_room_rate.php?room_data=${encodeURIComponent(roomQuery)}`, {
          method: 'GET',
    ...
    async function populateRoomTypes(targetSelects = null) {
      try {
        const response = await fetch('api/fetch_options.php?field_type=room_types');
        if (!response.ok) throw new Error(`Network response was not ok: ${response.status}`);
    ...
    document.getElementById('add-field-form').addEventListener('submit', async (e) => {
      ...
      try {
        const response = await fetch('api/create_option.php', {
          method: 'POST',
          body: formData
        });
        const result = await response.json();
    ...
    document.getElementById('services-form').addEventListener('submit', async (e) => {
      ...
      try {
        const response = await fetch('api/save_guest.php', {
          method: 'POST',
          body: formData
        });
        const result = await response.json();
