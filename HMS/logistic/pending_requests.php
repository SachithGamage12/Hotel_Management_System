...
if (isset($_POST['mark_printed'])) {
    ...
    try {
        $stmt = $conn->prepare("UPDATE item_requests SET status = 'printed' WHERE id = ? AND status = 'pending'");
        if (!$stmt) {
...
<button onclick="window.location.href='../logistic.php'" 
    ...
    Back
        ...
        <div id="print-error" class="alert alert-danger alert-dismissible fade show" role="alert">
        ...
        document.querySelectorAll('.print-button').forEach(button => {
            button.addEventListener('click', function() {
                ...
                try {
                    ...
                    alert('Error: Failed to extract request data. Details: ' + e.message);
                    return;
                }

                const loggedInUser = '<?php echo isset($_SESSION['username']) ? addslashes(htmlspecialchars($_SESSION['username'])) : ''; ?>';
                if (!loggedInUser) {
                    console.error('Loaded-in user not found in session.');
                ...
                const markAsPrinted = () => {
                    console.log('Marking request as printed:', requestId);
                    ...
                    .then(data => {
                        if (data.success) {
                            console.log('Request marked as printed. Reloading page.');
                            window.location.reload();
                        } else {
                            console.error('Error marking request as printed:', data.error);
                            alert('Error marking request as printed: ' + (data.error || 'Unknown error'));
                        }
                    })
                    .catch(error => {
                        console.error('AJAX error:', error);
                        alert('Error marking request as printed: ' + error);
                    });
                };
                ...
                setTimeout(() => {
                    if (typeof qz === 'undefined') {
                        ...
                        printErrorMessage.textContent = 'QZ Tray not loaded. Falling back to browser print. Status updated.';
                    ...
                    qz.websocket.connect().then(() => {
                        ...
                        return qz.printers.find('POSPrinter POS-80C').catch(err => {
                            console.error('Printer "POSPrinter POS-80C" not found. Falling back to default printer.');
                        ...
                        return qz.print(config, data).catch(err => {
                            ...
                            throw new Error('Print job failed: ' + err.message);
                        });
                    }).then(() => {
                        ...
                        printErrorMessage.textContent = err.message + '. Status updated.';
                        printError.style.display = 'block';
                        window.print();
                        setTimeout(() => {
                            console.log('Hiding print error after 5 seconds.');
