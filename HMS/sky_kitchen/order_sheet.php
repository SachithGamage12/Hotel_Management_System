...
<body class="bg-gradient-to-br from-teal-100 to-blue-200 min-h-screen p-6">
    <button onclick="window.location.href='../sky.php'" style="background-color: #00abf5ff; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; ">
        Back
    </button>
    <div class="container mx-auto max-w-6xl bg-white p-10 rounded-3xl shadow-2xl">
        <h2 class="text-4xl font-bold text-teal-900 mb-10 text-center">Create SKY/R Order Sheet</h2>
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_request'])) {
                ...
                try {
                    ...
                    try {
                        ...
                        $stmt = $conn->prepare("SELECT last_order_sheet_no FROM skyorder_sheet_counter WHERE id = 1 FOR UPDATE");
                        $stmt->execute();
                        $result = $stmt->fetch(PDO::FETCH_ASSOC);
                        if (!$result) {
                            // Initialize counter if not exists
                            $stmt = $conn->prepare("INSERT INTO skyorder_sheet_counter (id, last_order_sheet_no) VALUES (1, 1099)");
                            $stmt->execute();
                            $order_sheet_no = 1100;
                        } else {
                        ...
                        // Update counter
                        $stmt = $conn->prepare("INSERT INTO skyorder_sheet_counter (id, last_order_sheet_no) 
                                              VALUES (1, ?) 
                        ...
                        $stmt = $conn->prepare("INSERT INTO skyorder_sheet (item_id, requested_qty, status, request_date, order_sheet_no, responsible_id, function_type, function_date, day_night) 
                                              VALUES (?, ?, 'pending', NOW(), ?, ?, ?, ?, ?)");
                        foreach ($items as $item) {
                            // Verify item exists
                        ...
                        if ($function_type === 'SKY Buffer Stock Refill') {
                            $update_stmt = $conn->prepare("UPDATE skykitchen_buffer SET remaining_quantity = 0 WHERE item_id = ?");
                            foreach ($items as $item) {
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <select id="function_type" name="function_type" required class="block w-full max-w-md p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 sm:text-sm transition duration-200">
                        ...
                        <option value="SKY Restaurant">SKY Restaurant</option>
                        <option value="SKY Function">SKY Night Function</option>
                        <option value="SKY Birthday Function">SKY Birthday Function</option>
                        <option value="SKY Buffer Stock Refill">SKY Buffer Stock Refill</option>
                    </select>
                </div>
                <div>
    ...
    <script>
        ...
        functionTypeSelect.addEventListener('change', function() {
            ...
            if (this.value === 'SKY Buffer Stock Refill') {
        ...
        function fetchPendingOrders() {
            fetch('fetch_pending_skykitchen_orders.php')
                .then(response => {
        ...
        searchInput.addEventListener('input', function() {
            ...
            if (functionTypeSelect.value === 'SKY Buffer Stock Refill') return; // Skip search for buffer stock
        ...
        addItemButton.addEventListener('click', function() {
            ...
            if (functionTypeSelect.value === 'SKY Buffer Stock Refill') {
