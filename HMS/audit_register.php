...
$lock_result = $conn->query("SELECT is_locked FROM auidtregistration_access LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['unlock_registration'])) {
    ...
    if (empty($username) || empty($password)) {
        ...
        if (!$conn) {
            ...
            $stmt = $conn->prepare("SELECT id FROM audit_users WHERE username = ?");
            if ($stmt === false) {
                ...
                if ($result->num_rows > 0) {
                    ...
                    // Insert new user
                    $insert = $conn->prepare("INSERT INTO audit_users (username, password) VALUES (?, ?)");
                    if ($insert === false) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['unlock_registration'])) {
    ...
    if (!$conn) {
        ...
        $admin_username = 'AuditAdmin';
        $admin_password = '123456';
        
        if ($username === $admin_username && $password === $admin_password) {
            $update = $conn->prepare("UPDATE auidtregistration_access SET is_locked = FALSE");
            if ($update === false) {
...
<body>
    ...
    <div class="login-container">
        ...
        <div class="register-link">
            ...
            <?php else: ?>
                <p>Already have an account? <a href="audit_login.php">Login here</a></p>
            <?php endif; ?>
        </div>
    </div>
