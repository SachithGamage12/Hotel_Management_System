...
<head>
    ...
    <title>Main Kitchen Order Sheet</title>
...
<body class="bg-gradient-to-br from-teal-100 to-blue-200 min-h-screen p-6">
    <div class="container mx-auto max-w-6xl bg-white p-10 rounded-3xl shadow-2xl">
        <h2 class="text-4xl font-bold text-teal-900 mb-10 text-center">Create Main Kitchen Order Sheet</h2>
        ...
        try {
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_request'])) {
                ...
                try {
                    ...
                    try {
                        ...
                        $stmt = $conn->prepare("SELECT last_order_sheet_no FROM order_sheet_counter WHERE id = 1 FOR UPDATE");
                        $stmt->execute();
                        $result = $stmt->fetch(PDO::FETCH_ASSOC);
                        if (!$result) {
                            // Initialize counter if not exists
                            $stmt = $conn->prepare("INSERT INTO order_sheet_counter (id, last_order_sheet_no) VALUES (1, 1099)");
                            $stmt->execute();
                            $order_sheet_no = 1100;
                        } else {
                        ...
                        // Update counter
                        $stmt = $conn->prepare("INSERT INTO order_sheet_counter (id, last_order_sheet_no) 
                                              VALUES (1, ?) 
                                              ON DUPLICATE KEY UPDATE last_order_sheet_no = ?");
                        $stmt->execute([$order_sheet_no, $order_sheet_no]);
                        
                        // Insert items into order_sheet with function_type, function_date, and day_night
                        $stmt = $conn->prepare("INSERT INTO order_sheet (item_id, requested_qty, status, request_date, order_sheet_no, responsible_id, function_type, function_date, day_night) 
                                              VALUES (?, ?, 'pending', NOW(), ?, ?, ?, ?, ?)");
                        foreach ($items as $item) {
                            // Verify item exists
                            ...
                            $stmt->execute([$item['item_id'], $item['requested_qty'], $order_sheet_no, $responsible_id, $function_type, $function_date, $day_night_value]);
                            error_log("Inserted item: ID={$item['item_id']}, Qty={$item['requested_qty']}, Order=$order_sheet_no, Function=$function_type, Date=$function_date, Day/Night=$day_night_value");
                        }
                        
                        // Update remaining_quantity to 0 for SKY Buffer Stock Refill items
                        if ($function_type === 'SKY Buffer Stock Refill') {
                            $update_stmt = $conn->prepare("UPDATE kitchen_buffer SET remaining_quantity = 0 WHERE item_id = ?");
                            foreach ($items as $item) {
                        ...
                        $print_items = [];
                        foreach ($items as $item) {
                            // Fetch item name and unit from inventory
                            $stmt = $conn->prepare("SELECT item_name, unit FROM inventory WHERE id = ?");
            ...
            // Fetch available items with units, no stock information
            $stmt = $conn->prepare("
            ...
            // Fetch buffer stock items for SKY Buffer Stock Refill, excluding items with remaining_quantity = 0
            $buffer_items = [];
            $stmt = $conn->prepare("
                SELECT b.item_id, i.item_name, i.unit, b.quantity, b.remaining_quantity, b.usage
                FROM kitchen_buffer b
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <select id="function_type" name="function_type" required class="block w-full max-w-md p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 sm:text-sm transition duration-200">
                        ...
                        <option value="HGG Ballroom Function">HGG Ballroom Function</option>
                        <option value="HGG Orchid Hall Function">HGG Orchid Hall Function</option>
                        <option value="HGG Ballroom Function">HGG Ballroom Function</option>
                        <option value="HGG Red Hall Function">HGG Red Hall Function</option>
                        <option value="HGG Staff Meal">HGG Staff Meal</option>
                        <option value="Sapthapadhi Function">Sapthapadhi Function</option>
                        <option value="Outdoor Function">Outdoor Function</option> 
                        <option value="Main/K Buffer Stock Refill">HGG/R Buffer Stock Refill</option>
                    </select>
                </div>
                <div>
            ...
            <div class="space-y-4">
                ...
                <div class="flex space-x-4">
                    <div class="flex-1">
                        ...
                        <input type="number" id="item_qty" min="1" step="1" class="block w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 sm:text-sm transition duration-200" placeholder="Enter quantity">
                    </div>
                    <div class="flex-1">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Unit</label>
                        <span id="item_unit" class="block w-full p-3 bg-gray-100 border border-gray-300 rounded-lg text-sm text-gray-700">-</span>
                    </div>
                </div>
    ...
    <script>
        ...
        const itemUnitDisplay = document.getElementById('item_unit');
        ...
        // Set minimum date for function_date to today (2025-08-20)
        const today = new Date();
        const todayFormatted = today.toISOString().split('T')[0]; // e.g., "2025-08-20"
        ...
        functionTypeSelect.addEventListener('change', function() {
            ...
            itemIdInput.value = '';
            itemQtyInput.value = '';
            itemUnitDisplay.textContent = '-';
            searchResults.classList.add('hidden');

            if (this.value === 'Main/K Buffer Stock Refill') {
        ...
        function fetchPendingOrders() {
            fetch('fetch_pending_kitchen_orders.php')
                .then(response => {
        ...
        searchInput.addEventListener('input', function() {
            ...
            if (functionTypeSelect.value === 'Main/K Buffer Stock Refill') return; // Skip search for buffer stock
                ...
                if (filteredItems.length > 0) {
                    ...
                    filteredItems.forEach(item => {
                        ...
                        div.addEventListener('click', function() {
                            ...
                            itemUnitDisplay.textContent = item.unit || 'Unit';
                            itemQtyInput.value = '';
                            searchResults.classList.add('hidden');
                        });
        ...
        addItemButton.addEventListener('click', function() {
            ...
            if (functionTypeSelect.value === 'Main/K Buffer Stock Refill') {
                alert('Items for Main Kitchen Stock Refill are automatically populated and cannot be manually added.');
                return;
            }
            ...
            const requestedQty = parseInt(itemQtyInput.value);
            const existingItemIndex = selectedItems.findIndex(i => i.item_id == item.item_id);
            
            if (existingItemIndex >= 0) {
                // Update existing item's quantity
                selectedItems[existingItemIndex].requested_qty += requestedQty;
                console.log('Updated item quantity:', item.item_name, selectedItems[existingItemIndex].requested_qty, item.unit);
            } else {
                // Add new item
                selectedItems.push({
                    item_id: item.item_id,
                ...
                console.log('Added new item:', item.item_name, requestedQty, item.unit);
            }
