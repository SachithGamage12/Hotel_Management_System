...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_invoice') {
    try {
        ...
        $stmt = $conn->query("SELECT last_invoice_number FROM room_invoice_counter ORDER BY id DESC LIMIT 1");
        $last_invoice = $stmt->fetchColumn();
        $new_invoice_number = 'INV-' . ($last_invoice + 1);

        // Update invoice counter
        $stmt = $conn->prepare("INSERT INTO room_invoice_counter (last_invoice_number) VALUES (:last_invoice_number)");
        ...
        $stmt = $conn->prepare("
            INSERT INTO room_payments (
                ...
                value_type, amount_type, total_amount, advance_payment, 
                pending_amount, issued_by, nic, contact_no, payment_date
            ) VALUES (
                :booking_reference, :invoice_number, :ac_type, :meal_plan, 
                :value_type, :amount_type, :total_amount, :advance_payment, 
                :pending_amount, :issued_by, :nic, :contact_no, NOW()
            )
        ");
        ...
        $pending_amount = floatval(preg_replace('/[^0-9.]/', '', $_POST['pending_amount'] ?? $total_rate));
        $total_amount = floatval($total_rate);

        // Validate inputs
        if ($advance_payment < 0 || $pending_amount < 0) {
            throw new Exception("Advance or pending amount cannot be negative.");
        }
        if ($pending_amount + $advance_payment != $total_amount) {
            error_log("Validation error: pending_amount ($pending_amount) + advance_payment ($advance_payment) != total_amount ($total_amount)");
            throw new Exception("Invalid payment amounts: total does not match.");
        }

        // Log values for debugging
        error_log("Saving invoice: grc_number=$grc_number, total_amount=$total_amount, advance_payment=$advance_payment, pending_amount=$pending_amount");
...
<body>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        advancePaymentInput.addEventListener('input', function() {
            const total = parseFloat('<?php echo $total_rate; ?>') || 0;
            let payment = parseFloat(advancePaymentInput.value) || 0;
            if (payment < 0) {
                advancePaymentInput.value = '0';
                showError('Advance payment cannot be negative');
                payment = 0;
            } else if (payment > total) {
                advancePaymentInput.value = total.toFixed(2);
                showError('Advance payment cannot exceed total amount');
                payment = total;
            }
        ...
        function calculatePendingAmount() {
            ...
            advancePaymentDisplay.textContent = payment > 0 ? 'Rs. ' + payment.toFixed(2) : 'Rs. 0.00';
            pendingAmountDisplay.textContent = pending >= 0 ? 'Rs. ' + pending.toFixed(2) : 'N/A';
            
            // Show/hide payment rows based on advance payment
            const showPaymentRows = payment > 0;
            document.querySelectorAll('.advance-payment-row, .pending-amount-row').forEach(row => {
                row.style.display = showPaymentRows ? 'flex' : 'none';
            });
        }
