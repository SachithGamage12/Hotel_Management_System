...
try {
    ...
    // Create footnotes table if not exists
    $conn->exec("CREATE TABLE IF NOT EXISTS footnotes (
    ...
    if (isset($_POST['new_note']) && !empty($_POST['new_note'])) {
        $stmt = $conn->prepare("INSERT INTO footnotes (username, note, timestamp) VALUES (?, ?, NOW())");
        $stmt->execute([$username, $_POST['new_note']]);
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    }

    // Handle remove footnote
    if (isset($_POST['remove_id'])) {
        $stmt = $conn->prepare("DELETE FROM footnotes WHERE id = ? AND username = ?");
        $stmt->execute([$_POST['remove_id'], $username]);
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    }
    ...
    <style>
        ...
        * {
        ...
        .footnotes-section li {
            background: #fff59d; /* highlighted yellow */
            padding: 10px;
            margin-bottom: 10px;
            border-radius: 8px;
            display: flex;
        ...
        [data-theme="dark"] .footnotes-section li {
            background: #4a4a00; /* darker highlight for dark mode */
        }
        .footnotes-section form {
            margin-top: 20px;
        }
        .footnotes-section input[type="text"] {
            padding: 8px;
            width: 300px;
            border: 1px solid var(--primary);
            border-radius: 4px;
        }
        .footnotes-section button {
            padding: 8px 16px;
            background: var(--primary);
            color: white;
            border: none;
    ...
    <div class="admin-panel">
        <div class="sidebar">
            ...
            <div class="menu">
                ...
                <div class="menu-item" onclick="window.location.href='audit/function_payment.php'">
                    <i class="fas fa-credit-card"></i>
                    <span>Function Payments</span>
                </div>
                <div class="menu-item" onclick="window.location.href='audit_menu.php'">
                    <i class="fas fa-clipboard-list"></i>
                    <span>Function Menus</span>
                </div>
                <div class="menu-item" onclick="window.location.href='audit/supplier_payment.php'">
                    <i class="fas fa-receipt"></i>
                    <span>Supplier Payments</span>
                </div>
                 <div class="menu-item" onclick="window.location.href='audit/room_sale.php'">
                    <i class="fas fa-bed"></i>
                    <span>Room Payments</span>
                </div>
                <div class="menu-item" onclick="window.location.href='audit/back_office_room_bill.html'">
                    <i class="fas fa-file-invoice"></i>
                    <span>Back/Off Room Bill</span>
                </div>
              <div class="menu-item" onclick="window.location.href='audit/front_office_room_bill.php'">
                    <i class="fas fa-file-invoice"></i>
                    <span>Front/Off Room Bill</span>
                </div>
                <div class="menu-item" onclick="window.location.href='audit/main_ordersheet.php'">
                    <i class="fas fa-clipboard-list"></i>
                    <span>M/K Ordersheets</span>
                </div>
                <div class="menu-item" onclick="window.location.href='audit/hgg_res_ordersheet.php'">
                    <i class="fas fa-clipboard-list"></i>
                    <span>HGG B/R Ordersheets</span>
                </div>
                <div class="menu-item" onclick="window.location.href='audit/sky_ordersheet.php'">
                    <i class="fas fa-clipboard-list"></i>
                    <span>Sky/R Ordersheets</span>
                </div>
                <div class="menu-item" onclick="window.location.href='audit/stores_po.php'">
                    <i class="fas fa-shopping-bag"></i>
                    <span>Stores Purchased </span>
                </div>
                <div class="menu-item" onclick="window.location.href='audit/stores_grn.php'">
                    <i class="fas fa-receipt"></i>
                    <span>Stores GRN History</span>
                </div>
                <div class="menu-item" onclick="window.location.href='audit/logistic_po.php'">
                    <i class="fas fa-shopping-bag"></i>
                    <span>Logistic Purchased </span>
                </div>
                <div class="menu-item" onclick="window.location.href='audit/logistic_grn.php'">
                    <i class="fas fa-receipt"></i>
                    <span>Logistic GRN History</span>
                </div>
                <div class="menu-item" onclick="window.location.href='audit_logout.php'">
        ...
        <div class="main-content">
            ...
            <div class="dashboard">
                ...
                <div class="footnotes-section">
                    ...
                    <ul>
                        <?php
                        $stmt = $conn->prepare("SELECT id, note, timestamp FROM footnotes WHERE username = ? ORDER BY timestamp DESC");
                        $stmt->execute([$username]);
                        foreach ($stmt->fetchAll() as $row) {
                            echo "<li>" . htmlspecialchars($row['note']) . " <small>(" . $row['timestamp'] . ")</small> 
                    ...
                    <form method="POST">
                        <input type="text" name="new_note" placeholder="Add new footnote" required>
