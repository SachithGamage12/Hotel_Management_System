...
$lock_result = $conn->query("SELECT is_locked FROM storesregistration_access LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['unlock_registration'])) {
    ...
    if (empty($username) || empty($password)) {
        ...
        if (!$conn) {
            ...
            $stmt = $conn->prepare("SELECT id FROM store_users WHERE username = ?");
            if ($stmt === false) {
                ...
                if ($result->num_rows > 0) {
                    ...
                    // Insert new user
                    $insert = $conn->prepare("INSERT INTO store_users (username, password) VALUES (?, ?)");
                    if ($insert === false) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['unlock_registration'])) {
    ...
    if (!$conn) {
        ...
        $admin_username = 'StoreAdmin';
        $admin_password = '1234';
        
        if ($username === $admin_username && $password === $admin_password) {
            $update = $conn->prepare("UPDATE storesregistration_access SET is_locked = FALSE");
            if ($update === false) {
...
<body>
    ...
    <div class="login-container">
        ...
        <form action="stores_register.php" method="POST">
        ...
        <div class="register-link">
            ...
            <?php else: ?>
                <p>Already have an account? <a href="stores_login.php">Login here</a></p>
            <?php endif; ?>
        </div>
    </div>
