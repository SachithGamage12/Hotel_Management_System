...
<body>
    <h2>Logistics Approver Registration</h2>
    <?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($username) || empty($password) || empty($confirm_password)) {
            ...
            $stmt = $conn->prepare("SELECT id FROM approvers WHERE username = ?");
            ...
            if ($stmt->num_rows > 0) {
                ...
                if ($stmt->execute()) {
                    $success = "Approver registered successfully!";
                } else {
    ...
    <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        ...
        <button type="submit">Register Approver</button>
    </form>
</body>
</html>