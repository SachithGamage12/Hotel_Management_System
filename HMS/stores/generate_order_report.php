...
class OrderSheetReport extends TCPDF {
    ...
    public function __construct($db) {
        // Use portrait orientation for A4
    ...
    private function generateSummaryTable($data) {
        ...
        $this->Cell(25, 6, 'Item Name', 1, 0, 'C');
        $this->Cell(20, 6, 'Requested Qty', 1, 0, 'C');
        $this->Cell(20, 6, 'Issued Qty', 1, 0, 'C');
        $this->Cell(15, 6, 'Status', 1, 0, 'C');
        $this->Cell(20, 6, 'Request Date', 1, 0, 'C');
        $this->Cell(15, 6, 'Order Sheet No', 1, 0, 'C');
        $this->Cell(25, 6, 'Responsible', 1, 0, 'C');
        $this->Cell(20, 6, 'Function Type', 1, 0, 'C');
        $this->Cell(15, 6, 'Function Date', 1, 0, 'C');
        ...
        foreach ($data as $row) {
            ...
            $this->MultiCell(25, 5, $row['item_name'], 1, 'L', false, 0);
            $this->MultiCell(20, 5, $req_qty, 1, 'R', false, 0);
            $this->MultiCell(20, 5, $iss_qty, 1, 'R', false, 0);
            $this->MultiCell(15, 5, ucfirst($row['status']), 1, 'C', false, 0);
            $this->MultiCell(20, 5, $row['request_date'], 1, 'C', false, 0);
            $this->MultiCell(15, 5, $row['order_sheet_no'], 1, 'C', false, 0);
            $this->MultiCell(25, 5, $row['responsible_name'], 1, 'L', false, 0);
            $this->MultiCell(20, 5, $row['function_type'], 1, 'L', false, 0);
            $this->MultiCell(15, 5, $row['function_date'] ?? 'N/A', 1, 'C', false, 0);
    ...
    private function generateTotals($data) {
        ...
        $this->Cell(100, 6, 'Total Requested Quantity:', 0, 0, 'R');
        $this->Cell(30, 6, number_format($totalRequested, 2), 0, 1, 'R');
        
        $this->Cell(100, 6, 'Total Issued Quantity:', 0, 0, 'R');
