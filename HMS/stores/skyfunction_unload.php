        ...
        try {
            foreach ($items as $item_id => $item) {
                if (isset($item['remaining_qty']) && $item['remaining_qty'] !== '') {
                    ...
                    $stmt = $conn->prepare("
                        INSERT INTO skyfunction_unload 
                    ...
                    $history_stmt = $conn_history->prepare("
                        INSERT INTO skyfunction_unload_history 
                        (order_sheet_no, item_id, requested_qty, issued_qty, remaining_qty, usage_qty, 
                         unload_date, function_type, day_night, created_by)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ");
            ...
            header("Location: skyunload_history.php?date=" . urlencode($unload_date));
            exit();
        } catch (Exception $e) {
            $conn->rollBack();
...
<head>
    ...
    <title>SKY Restaurant Unload System</title>
    ...
    <div class="container mx-auto max-w-7xl">
        <header class="mb-8">
            <h1 class="text-4xl font-bold text-teal-800 text-center flex items-center justify-center gap-2">
                <i class="fas fa-truck-loading"></i> SKY/R Unload System
            </h1>
        </header>
