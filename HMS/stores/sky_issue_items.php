        ...
        function fulfillPendingOrders($conn, $item_id, $today) {
            ...
            foreach ($pending_orders as $key => $order) {
                ...
                if ($new_issued_qty + $alloc_unl >= $order['requested_qty']) {
                    ...
                    $stmt = $conn->prepare("
                        INSERT INTO skyorder_sheet (order_sheet_no, item_id, requested_qty, issued_qty, status, responsible_id, request_date, function_type, function_date, day_night)
            ...
            if ($used_unload > 0) {
                ...
                while ($remaining_deduct > 0 && $row = $result->fetch_assoc()) {
                    ...
                    $update_stmt = $conn->prepare("UPDATE skyfunction_unload SET remaining_qty = ? WHERE id = ?");
...
function fetchPendingOrders() {
    ...
    fetch('skyfetch_pending_orders.php', {
        cache: 'no-store',
        headers: {
