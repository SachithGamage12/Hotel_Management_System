...
<body class="bg-gradient-to-br from-teal-100 to-blue-200 min-h-screen p-6">
    ...
    <div class="container mx-auto max-w-6xl bg-white p-10 rounded-3xl shadow-2xl">
        <h2 class="text-4xl font-bold text-teal-900 mb-10 text-center">Issue Stock for HGG Main Kitc Orders</h2>
        ...
        function fulfillPendingOrders($conn, $item_id, $today) {
            ...
            foreach ($pending_orders as $key => $order) {
                ...
                if ($new_issued_qty + $alloc_unl >= $order['requested_qty']) {
                    ...
                    $stmt = $conn->prepare("
                        INSERT INTO order_sheet (order_sheet_no, item_id, requested_qty, issued_qty, status, responsible_id, request_date, function_type, function_date, day_night)
            ...
            if ($used_unload > 0) {
                ...
                while ($remaining_deduct > 0 && $row = $result->fetch_assoc()) {
                    ...
                    $update_stmt = $conn->prepare("UPDATE function_unload SET remaining_qty = ? WHERE id = ?");
            ...
            const printContent = `
                ...
                <body>
                    <div class='receipt-container'>
                        <div class='receipt-header'>
                            <h2>Issued Order Sheet <br> HGG Main Kitchen</h2>
                            <div><strong>Function Date</strong><span class='colon'>:</span><span>${document.getElementById('print_function_date').textContent}</span></div>
