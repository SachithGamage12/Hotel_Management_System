...
$min_date_stmt = $pdo->query("SELECT MIN(date) FROM grn_records");
$min_date = $min_date_stmt->fetchColumn();
$min_year = $min_date ? date('Y', strtotime($min_date)) : (date('Y') - 5);

$max_date_stmt = $pdo->query("SELECT MAX(date) FROM grn_records");
    ...
    switch ($report_type) {
        case 'daily':
            $stmt = $pdo->prepare("
                SELECT r.id AS grn_id, r.grn_number, r.date, r.location, r.received_by, r.checked_by,
                       i.id AS item_id, i.item_name, i.quantity, i.unit
                FROM grn_records r
                LEFT JOIN grn_items i ON r.id = i.grn_id
                WHERE DATE(r.date) = :selected_date
                ORDER BY r.date DESC, r.id, i.id
            ");
        ...
        case 'monthly':
            ...
            $stmt = $pdo->prepare("
                ...
                GROUP BY DATE(r.date), r.id, i.id
                ORDER BY report_date DESC, r.id, i.id
            ");
        ...
        case 'yearly':
            ...
            $stmt = $pdo->prepare("
                ...
                LEFT JOIN grn_items i ON r.id = i.grn_id
                WHERE r.date BETWEEN :start_date AND :end_date
                GROUP BY MONTH(r.date), r.id, i.id
                ORDER BY month DESC, r.id, i.id
            ");
            $stmt->execute(['start_date' => $start_date, 'end_date' => $end_date]);
            break;
