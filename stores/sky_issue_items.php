...
.print-modal-content {
    width: 80mm; /* Fixed width for printing */
    max-width: 90vw; /* Responsive max-width for smaller screens */
    margin: 20px auto; /* Margin for spacing on smaller screens */
    padding: 10px;
    background: #fff;
    font-family: 'Arial', sans-serif;
    font-size: 9pt;
    ...
    border-radius: 0.5rem; /* Slight rounding for better appearance on screen */
}
...
.controls {
    text-align: center;
    margin: 10px 0; /* Increased margin for visibility */
    padding: 10px 0;
    border-top: 2px dashed #000;
    display: block !important; /* Ensure visibility on screen */
}

.controls button {
    padding: 5px 10px; /* Slightly larger padding for better click area */
    margin: 0 5px;
    font-size: 10pt; /* Slightly larger font for readability */
    cursor: pointer;
    background: #f0f0f0;
...
@media screen and (max-width: 600px) {
    .print-modal-content {
        ...
        padding: 15px;
        font-size: 8pt; /* Slightly smaller font for mobile */
    }

    .receipt-table th, .receipt-table td {
        padding: 1mm 0.5mm; /* Reduced padding for mobile */
    }

    .receipt-table th:nth-child(1),
    .receipt-table td:nth-child(1) {
        width: 25%; /* Adjust column widths for mobile */
    }
    ...
    .signatures {
        flex-direction: column; /* Stack signatures vertically on small screens */
        align-items: center;
    }
    ...
    .controls button {
        width: 45%; /* Make buttons wider on mobile */
        padding: 8px;
        font-size: 9pt;
    }
}
...
<body class="bg-gradient-to-br from-teal-100 to-blue-200 min-h-screen p-6">
    ...
    <div class="container mx-auto max-w-6xl bg-white p-10 rounded-3xl shadow-2xl">
        <h2 class="text-4xl font-bold text-teal-900 mb-10 text-center">Issue Stock for SKY/R Orders</h2>
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['issue_all'])) {
            ...
            error_log("Function Date: $function_date, Responsible ID: $responsible_id, Today: $today");
            ...
            try {
                ...
                // Fetch unload quantities for each item
                ...
                if ($unload_result) {
                    while ($unload_row = $unload_result->fetch_assoc()) {
                        $item_unload_quantities[$unload_row['item_id']] = floatval($unload_row['unload_qty']);
                    }
                } else {
                    throw new Exception("Failed to fetch unload quantities: " . $conn->error);
                }

                // Fetch order details
                $stmt = $conn->prepare("
                    SELECT os.order_sheet_no, os.item_id, os.requested_qty, os.issued_qty, os.unload_issued_qty, i.item_name, i.unit, os.function_type, os.function_date, os.day_night
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    $unload_qty = isset($item_unload_quantities[$item_id]) ? floatval($item_unload_quantities[$item_id]) : 0;
                    ...
                    $items_by_id[$item_id]['unload_issued_qty'] += floatval($row['unload_issued_qty']);
                    $items_by_id[$item_id]['order_sheets'][] = [
                        ...
                        'unload_issued_qty' => floatval($row['unload_issued_qty'])
                    ];
                ...
                foreach ($items_by_id as $item_id => &$item) {
                    error_log("Processing item_id: $item_id, item_name: {$item['item_name']}, requested_qty: {$item['requested_qty']}, issued_qty: {$item['issued_qty']}, unload_issued_qty: {$item['unload_issued_qty']}, unload_qty: {$item['unload_qty']}");
                    ...
                    foreach ($item['order_sheets'] as $order) {
                        ...
                        $order_balance = floatval($order['requested_qty']) - floatval($order['issued_qty']) - floatval($order['unload_issued_qty']);
                        
                        if ($order_balance <= 0) {
                            continue;
                        }

                        $available_unload = isset($item['unload_qty']) ? floatval($item['unload_qty']) : 0;
                        
                        if ($order_balance <= $available_unload) {
                            // Case 1: Requested quantity is less than or equal to unload quantity
                            $used_unload = $order_balance;
                            $issued_from_stock = 0;

                            if ($used_unload > 0) {
                                $remaining_unload_to_use = $used_unload;
                                $unload_stmt = $conn->prepare("
                                    ...
                                    ORDER BY id ASC
                                ");
                                $unload_stmt->bind_param("i", $item_id);
                                $unload_stmt->execute();
                                $unload_result = $unload_stmt->get_result();
                                
                                while ($remaining_unload_to_use > 0 && $unload_row = $unload_result->fetch_assoc()) {
                                    $unload_id = $unload_row['id'];
                                    $batch_unload_qty = floatval($unload_row['remaining_qty']);
                                    $use_from_batch = min($remaining_unload_to_use, $batch_unload_qty);
                                    $new_unload_qty = $batch_unload_qty - $use_from_batch;
                                    
                                    // Update unload quantity
                                    $update_unload_stmt = $conn->prepare("
                                        UPDATE skyfunction_unload 
                                        SET remaining_qty = ?, last_used_date = ? 
                                        WHERE id = ?
                                    ");
                                    $update_unload_stmt->bind_param("dsi", $new_unload_qty, $today, $unload_id);
                                    $update_unload_stmt->execute();
                                    $update_unload_stmt->close();
                                    
                                    $remaining_unload_to_use -= $use_from_batch;
                                    error_log("Used $use_from_batch from unload batch $unload_id, new remaining: $new_unload_qty");
                                }
                                $unload_stmt->close();
                            }
                            
                            // Update skyorder_sheet 
                            $new_issued_qty = floatval($order['issued_qty']) + $issued_from_stock;
                            $new_unload_issued_qty = floatval($order['unload_issued_qty']) + $used_unload;
                            $stmt = $conn->prepare("
                                UPDATE skyorder_sheet 
                                SET status = 'issued', issued_qty = ?, unload_issued_qty = ?
                                WHERE order_sheet_no = ? AND item_id = ?
                            ");
                            $stmt->bind_param("ddsi", $new_issued_qty, $new_unload_issued_qty, $order_sheet_no, $item_id);
                            $stmt->execute();
                            $stmt->close();
                            
                            error_log("Order $order_sheet_no: Fully fulfilled by unload only - unload: $used_unload, stock: 0");
                            ...
                            $issued_items[] = [
                                ...
                                'issued_qty' => $issued_from_stock,
                                'unload_qty' => $used_unload,
                                'unit' => $item['unit']
                            ];
                        } else {
                            // Case 2: Requested quantity is greater than unload quantity
                            $used_unload = $available_unload;
                            $needed_from_stock = $order_balance - $used_unload;
                            ...
                            $issued_from_stock = min($needed_from_stock, $total_available_stock);

                            // Deduct from unload if available
                            if ($used_unload > 0) {
                                $remaining_unload_to_use = $used_unload;
                                $unload_stmt = $conn->prepare("
                                    SELECT id, remaining_qty 
                            ...
                            if ($remaining_stock_to_issue > 0 && $regular_stock > 0) {
                                ...
                                while ($remaining_stock_to_issue > 0 && $row = $result->fetch_assoc()) {
                                    $batch_id = $row['id'];
                                    ...
                                    $update_stmt->bind_param("di", $new_stock, $batch_id);
                                    $update_stmt->execute();
                                    $update_stmt->close();
                            ...
                            if ($remaining_stock_to_issue > 0 && $buffer_stock > 0) {
                                ...
                                $stmt->bind_param("di", $new_buffer_stock, $item_id);
                                $stmt->execute();
                                $stmt->close();
                            ...
                            $new_unload_issued_qty = floatval($order['unload_issued_qty']) + $used_unload;
                            $remaining_for_order = floatval($order['requested_qty']) - $new_issued_qty - $new_unload_issued_qty;
                            ...
                            if ($remaining_for_order <= 0) {
                                ...
                                $stmt->execute();
                                $stmt->close();
                                error_log("Order $order_sheet_no: Fully issued - unload: $used_unload, stock: $actual_issued_from_stock");
                            } else {
                                // Partially issued
                                $stmt = $conn->prepare("
                                    UPDATE skyorder_sheet 
                                    SET issued_qty = ?, unload_issued_qty = ?, status = 'pending'
                                    WHERE order_sheet_no = ? AND item_id = ?
                                ");
                                $stmt->bind_param("ddsi", $new_issued_qty, $new_unload_issued_qty, $order_sheet_no, $item_id);
                                $stmt->execute();
                                ...
                                error_log("Order $order_sheet_no: Partially issued - unload: $used_unload, stock: $actual_issued_from_stock, remaining: $remaining_for_order");
                            }
                ...
                if (!empty($partial_items)) {
                    ...
                    foreach ($partial_items as $item) {
                        $partial_messages[] = "{$item['item_name']}: Available {$item['total_available']}, Requested {$item['requested_qty']}, Issued {$item['issued_qty']}, Unload {$item['unload_qty']}, Remaining {$item['remaining_qty']} (Order: {$item['order_sheet_no']})";
                    }
        ...
        $result = $conn->query("
            SELECT DISTINCT os.function_date, os.order_sheet_no, os.item_id, i.item_name, 
                   os.requested_qty, os.issued_qty, os.unload_issued_qty, i.unit, 
        ...
        if ($unload_result) {
            while ($unload_row = $unload_result->fetch_assoc()) {
                $item_unload_quantities[$unload_row['item_id']] = floatval($unload_row['remaining_qty']);
            }
        } else {
        ...
        if ($result) {
            while ($row = $result->fetch_assoc()) {
                ...
                $order_sheet_no = $row['order_sheet_no'];
                $item_id = $row['item_id'];
                $remaining_qty = isset($item_unload_quantities[$item_id]) ? floatval($item_unload_quantities[$item_id]) : 0;
                ...
                $orders_by_date[$function_date]['items_by_id'][$item_id]['requested_qty'] += floatval($row['requested_qty']);
                $orders_by_date[$function_date]['items_by_id'][$item_id]['issued_qty'] += floatval($row['issued_qty']);
                $orders_by_date[$function_date]['items_by_id'][$item_id]['unload_issued_qty'] += floatval($row['unload_issued_qty']);
            ...
            <div class="print-modal-content">
                <div class="receipt-container">
                    <div class="receipt-header">
                        <h2>Issued Order Sheet <br>SKY RESTAURANT</h2>
                        <div><strong>Function Date</strong><span class="colon">:</span><span id="print_function_date"></span></div>
                            ...
                            foreach ($orders_by_date as $date => $date_order): ?>
                                <?php foreach ($date_order['order_sheets'] as $order): ?>
                                    <tr class="<?php echo $index++ % 2 ? 'bg-gray-50' : 'bg-white'; ?> hover:bg-teal-50">
                                        ...
                                        <td class="py-3 px-4 border-b text-sm"><?php echo htmlspecialchars(implode(', ', array_map(function($item) { return $item['unload_qty'] !== null ? $item['unload_qty'] : '0'; }, $order['items']))); ?></td>
                                        <td class="py-3 px-4 border-b text-sm"><?php 
                                            $needed_quantities = array_map(function($item) { 
                                                $unload_issued = floatval($item['unload_issued_qty']);
                                                $unload_avail = $item['unload_qty'] !== null ? floatval($item['unload_qty']) : 0;
                                                return max(0, (floatval($item['requested_qty']) - floatval($item['issued_qty']) - $unload_issued - $unload_avail));
                                            }, $order['items']);
    ...
    Object.values(orders).forEach(date_order => {
        Object.values(date_order.order_sheets).forEach(order => {
            ...
            const neededQuantities = order.items.map(item => {
                const unloadIssued = item.unload_issued_qty || 0;
                const unloadAvail = item.unload_qty !== null ? item.unload_qty : 0;
                return Math.max(0, (item.requested_qty - item.issued_qty - unloadIssued - unloadAvail));
            });
...
function fetchPendingOrders() {
    ...
    fetch(`skyfetch_pending_orders.php?_=${timestamp}`, {
        cache: 'no-store',
        headers: {
...
function showDateDetails(functionDate) {
    ...
    dateOrder.items_by_id.forEach(item => {
        console.log(`Processing item: ${item.item_name}, item_id: ${item.item_id}, requested_qty: ${item.requested_qty}, issued_qty: ${item.issued_qty}, unload_issued_qty: ${item.unload_issued_qty}, unload_qty: ${item.unload_qty}`);
        fetch(`check_stock.php?item_id=${item.item_id}&_=${new Date().getTime()}`, { cache: 'no-store' })
            ...
            .then(stockData => {
                ...
                const unloadIssued = item.unload_issued_qty || 0;
                const unloadAvail = item.unload_qty !== null && item.unload_qty !== undefined ? item.unload_qty : 0;
                const neededQty = Math.max(0, (item.requested_qty - item.issued_qty - unloadIssued - unloadAvail));
                ...
                row.innerHTML = `
                    ...
                    <td class="py-3 px-4 border-b text-sm">${unloadAvail}</td>
...
function showPasswordModalForAll() {
    ...
    fetch(`fetch_responsible_persons.php?_=${new Date().getTime()}`, { cache: 'no-store' })
        .then(response => {
            if (!response.ok) throw new Error('Network response was not ok');
            return response.json();
        })
...
function showPrintModal(order) {
    ...
    if (order.partial_items && order.partial_items.length > 0) {
        ...
        printPartialDetails.innerHTML = order.partial_items.map(item => {
            const unloadQty = item.unload_qty !== null && item.unload_qty !== undefined ? item.unload_qty : 0;
            return `<div>${item.item_name}: Requested ${item.requested_qty}, Issued ${item.issued_qty}, Unload ${unloadQty}, Remaining ${item.remaining_qty} (Order: ${item.order_sheet_no})</div>`;
        }).join('');
    } else {
