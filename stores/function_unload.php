...
try {
    ...
    $table_check = $conn->query("SHOW TABLES LIKE 'order_sheet'");
    if ($table_check->rowCount() == 0) {
        $error_message = "Error: Table 'order_sheet' does not exist in the database.";
    } else {
        // Debug: Check columns in order_sheet
        $column_check = $conn->query("SHOW COLUMNS FROM order_sheet LIKE 'status'");
        if ($column_check->rowCount() == 0) {
            $error_message = "Error: Column 'status' does not exist in the 'order_sheet' table.";
        }
    }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_unload'])) {
        ...
        try {
            foreach ($items as $item_id => $item) {
                if (isset($item['remaining_qty']) && $item['remaining_qty'] !== '') {
                    ...
                    $history_stmt = $conn_history->prepare("
                        INSERT INTO function_unload_history 
                        (order_sheet_no, item_id, requested_qty, issued_qty, remaining_qty, usage_qty, 
                         unload_date, function_type, day_night, created_by)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ");
            ...
            header("Location: unload_history.php?date=" . urlencode($unload_date));
            exit();
        } catch (Exception $e) {
            $conn->rollBack();
    ...
    // Debug: Fetch raw order_sheet data for the selected date
    $debug_stmt = $conn->prepare("
        SELECT os.item_id, i.item_name, os.requested_qty, os.issued_qty, os.status, os.function_date
        FROM order_sheet os
        JOIN inventory i ON os.item_id = i.id
        WHERE DATE(os.function_date) = ?
    ");
    ...
    if (empty($debug_items)) {
        $error_message = "No records found in order_sheet for date: " . htmlspecialchars($selected_date) . ". Please verify data exists.";
    } else {
        $debug_message = "Found " . count($debug_items) . " records in order_sheet for date: " . htmlspecialchars($selected_date);
    }
    ...
    $orders_stmt = $conn->prepare("
        SELECT 
            ...
            SUM(
                CASE 
                    WHEN LOWER(os.status) = 'issued' AND (os.issued_qty IS NULL OR os.issued_qty = 0) 
                    THEN os.requested_qty 
                    ELSE COALESCE(os.issued_qty, 0) 
                END
            ) AS total_issued,
    ...
    if (empty($aggregated_items)) {
        $error_message = "No aggregated items found for date: " . htmlspecialchars($selected_date) . ". Raw records: " . count($debug_items);
    }
...
<head>
    ...
    <title>Restaurant Unload System</title>
    ...
    <div class="container mx-auto max-w-7xl">
        <header class="mb-8">
            <h1 class="text-4xl font-bold text-teal-800 text-center flex items-center justify-center gap-2">
                <i class="fas fa-truck-loading"></i> /R Unload System
            </h1>
        </header>
        ...
        <?php if (isset($debug_message)): ?>
            <div class="bg-blue-100 text-blue-800 p-4 rounded-lg mb-6 text-center flex items-center justify-center gap-2">
        ...
        <!-- Debug: Display raw order_sheet data -->
        <?php if (!empty($debug_items)): ?>
            <div class="card mb-8">
                <h2 class="text-2xl font-semibold text-teal-700 mb-4 flex items-center gap-2"><i class="fas fa-bug"></i> Debug: Raw Order Sheet Data</h2>
                <div class="table-container shadow-md rounded-lg overflow-hidden">
                    <table class="min-w-full bg-white">
                        <thead class="bg-teal-600 text-white">
                            <tr>
                                <th class="py-3 px-4 text-left text-sm font-medium">Item ID</th>
                                <th class="py-3 px-4 text-left text-sm font-medium">Item Name</th>
                                <th class="py-3 px-4 text-left text-sm font-medium">Requested Qty</th>
                                <th class="py-3 px-4 text-left text-sm font-medium">Issued Qty</th>
                                <th class="py-3 px-4 text-left text-sm font-medium">Status</th>
                                <th class="py-3 px-4 text-left text-sm font-medium">Function Date</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($debug_items as $item): ?>
                                <tr class="hover:bg-teal-50 transition">
                                    <td class="py-3 px-4 border-b text-sm"><?php echo htmlspecialchars($item['item_id']); ?></td>
                                    <td class="py-3 px-4 border-b text-sm"><?php echo htmlspecialchars($item['item_name']); ?></td>
                                    <td class="py-3 px-4 border-b text-sm"><?php echo number_format($item['requested_qty'], 2); ?></td>
                                    <td class="py-3 px-4 border-b text-sm"><?php echo is_null($item['issued_qty']) ? 'NULL' : number_format($item['issued_qty'], 2); ?></td>
                                    <td class="py-3 px-4 border-b text-sm"><?php echo htmlspecialchars($item['status']); ?></td>
                                    <td class="py-3 px-4 border-b text-sm"><?php echo htmlspecialchars($item['function_date']); ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
