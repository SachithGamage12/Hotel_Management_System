...
<body class="min-h-screen flex flex-col">
  ...
  <div class="w-full min-h-screen flex flex-col">
    ...
    <div class="header flex justify-between items-center">
      <img src="../image/rose logo png.png" alt="Hotel Logo" class="h-14 rounded-full animate-pulseGlow">
  ...
  <script>
    ...
    async function fetchNextGRCNumber() {
      try {
        const response = await fetch('../rapi/fetch_next_grc.php');
        if (!response.ok) {
    ...
    async function fetchRoomRates() {
      ...
      try {
        const response = await fetch(`../rapi/fetch_room_rate.php?room_data=${encodeURIComponent(roomQuery)}`, {
          method: 'GET',
    ...
    async function populateRoomTypes(targetSelects = null) {
      try {
        const response = await fetch('../rapi/fetch_options.php?field_type=room_types');
    ...
    async function populateSelect(selectId, fieldType) {
      try {
        const response = await fetch(`../rapi/fetch_options.php?field_type=${fieldType}`);
        if (!response.ok) throw new Error(`Network response was not ok: ${response.status}`);
        const items = await response.json();
    ...
    document.getElementById('add-field-form').addEventListener('submit', async (e) => {
      ...
      try {
        const response = await fetch('../rapi/create_option.php', {
          method: 'POST',
          body: formData
        });
        const result = await response.json();
    ...
    document.getElementById('services-form').addEventListener('submit', async (e) => {
      ...
      try {
        const response = await fetch('../rapi/save_guest.php', {
          method: 'POST',
          body: formData
        });
        const result = await response.json();
