...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['bill_number'])) {
    try {
        ...
        $stmt = $conn->prepare("
            ...
            FROM roseroom_payments rp
            LEFT JOIN roseguests g ON rp.booking_reference = g.grc_number
            LEFT JOIN meal_plans mp ON g.meal_plan_id = mp.id
            LEFT JOIN roseroom_invoice_counter ric ON rp.invoice_number = CONCAT('INV-', ric.last_invoice_number)
