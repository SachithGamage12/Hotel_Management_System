...
$lock_result = $conn->query("SELECT is_locked FROM roseforegistration_access LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['unlock_registration'])) {
    ...
    if (empty($username) || empty($password)) {
        ...
        if (!$conn) {
            ...
            $stmt = $conn->prepare("SELECT id FROM rosefo_users WHERE username = ?");
            if ($stmt === false) {
                ...
                if ($result->num_rows > 0) {
                    ...
                    // Insert new user
                    $insert = $conn->prepare("INSERT INTO rosefo_users (username, password) VALUES (?, ?)");
                    if ($insert === false) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['unlock_registration'])) {
    ...
    if (!$conn) {
        ...
        if ($username === $admin_username && $password === $admin_password) {
            $update = $conn->prepare("UPDATE roseforegistration_access SET is_locked = FALSE");
            if ($update === false) {
