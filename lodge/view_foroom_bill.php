...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['bill_number'])) {
    try {
        ...
        // Fetch invoice details including issued_by from lodgeroom_payments
        $stmt = $conn->prepare("
            SELECT rp.*, g.*, mp.name AS meal_plan_name
            FROM lodgeroom_payments rp
            LEFT JOIN lodgeguests g ON rp.booking_reference = g.grc_number
