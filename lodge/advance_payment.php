...
try {
    ...
    $stmt = $conn->query("SELECT DISTINCT room_number FROM lodgerooms");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'fetch_rate') {
    try {
        ...
        $stmt = $conn->prepare("
            ...
            FROM lodgeroom_rates rr
            JOIN lodgerooms r ON rr.room_number = r.room_number
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_advance_payment') {
    try {
        ...
        $stmt = $conn->query("SELECT COUNT(*) FROM lodgeadvance_invoice_counter WHERE id = 1");
        ...
        $stmt = $conn->query("SELECT last_invoice_number FROM lodgeadvance_invoice_counter WHERE id = 1 LIMIT 1 FOR UPDATE");
        ...
        $stmt = $conn->prepare("UPDATE lodgeadvance_invoice_counter SET last_invoice_number = ? WHERE id = 1");
        ...
        $stmt = $conn->prepare("
            INSERT INTO lodgeadvance_payments (
