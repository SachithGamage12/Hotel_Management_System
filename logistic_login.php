...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['unlock_registration'])) {
        ...
        if ($username === $admin_username && $password === $admin_password) {
            ...
            if ($update === false) {
                ...
                header("Location: logistic_register.php");
                exit();
            }
        } else {
    ...
    // Normal login
        ...
        $stmt = $conn->prepare("SELECT id, username, password FROM logi_users WHERE username = ?");
        if ($stmt === false) {
...
<body>
    ...
    <div class="login-container">
        <h1>Login to Logistic Panel</h1>
        
        <?php if ($error): ?>
            <div class="error"><?php echo htmlspecialchars($error); ?></div>
        <?php endif; ?>
        
        <form action="logistic_login.php" method="POST">
