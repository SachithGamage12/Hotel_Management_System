...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['bill_number'])) {
    try {
        ...
        if (!is_numeric($bill_number) || $bill_number <= 0) {
            throw new Exception("Invalid bill number provided.");
        }

        // Fetch invoice details including issued_by from room_payments
        $stmt = $conn->prepare("
            SELECT rp.*, g.*, mp.name AS meal_plan_name
        ...
        ");
        $invoice_number_input = sprintf('INV-%04d', $bill_number);
        $stmt->bindParam(':invoice_number', $invoice_number_input, PDO::PARAM_STR);
        $stmt->execute();
        $result = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$result) {
            $error = "No invoice found with bill number $bill_number (Invoice: $invoice_number_input).";
        } else {
            $guest = $result;
            $invoice_number = $result['invoice_number'];
            $total_rate = floatval($result['total_amount']);
            $discount = floatval($result['discount'] ?? 0);
            $final_total = $total_rate - $discount;
            $advance_payment = floatval($result['advance_payment'] ?? 0);
            $pending_amount = floatval($result['pending_amount'] ?? 0);
            $additional_hours = intval($result['additional_hours'] ?? 0);
            $hourly_rate = floatval($result['hourly_rate'] ?? 0.00);
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                ...
                foreach ($rooms as &$room) {
                    ...
                    $room['room_type_name'] = $room_types[$room['room_type']] ?? 'Unknown';
                    $room['rate'] = $rate_result['rate'] ?? 'N/A';
                ...
                error_log("Successfully loaded invoice: $invoice_number for bill number: $bill_number");
            }
        }
...
<body>
            ...
            <?php if ($guest): ?>
                <div class="payment-summary">
                    <div class="payment-row">
                        ...
                        <span><?php echo is_numeric($total_rate - $additional_charge) ? 'Rs. ' . number_format($total_rate - $additional_charge, 2) : 'N/A'; ?></span>
                    </div>
                    <div class="payment-row hours-count-row">
                        <span>Additional Hours:</span>
                        <span id="hoursCountDisplay"><?php echo htmlspecialchars($additional_hours); ?></span>
                    </div>
                    <div class="payment-row additional-hour-row">
                        <span>Additional Hours Charge:</span>
                        <span id="additionalHourChargeDisplay"><?php echo is_numeric($additional_charge) ? 'Rs. ' . number_format($additional_charge, 2) : 'Rs. 0.00'; ?></span>
                    </div>
                    <div class="payment-row">
                        <span>Subtotal:</span>
                    ...
                    <div class="payment-row discount-row">
                        <span>Discount:</span>
                        <span id="discountDisplay"><?php echo is_numeric($discount) ? 'Rs. ' . number_format($discount, 2) : 'Rs. 0.00'; ?></span>
                    </div>
                    <div class="payment-row total">
                        <span>Final Total:</span>
                        <span id="finalTotalDisplay"><?php echo is_numeric($final_total) ? 'Rs. ' . number_format($final_total, 2) : 'N/A'; ?></span>
                    </div>
        ...
        <div class="footer<?php echo $guest ? ' active' : ''; ?>">
            <div class="signature-section">
                <div class="signature-box">
                    <p>Bill Issued by: <span id="issuedBy">
                        <?php echo htmlspecialchars($guest['issued_by'] ?? 'Admin'); ?>
                    </span></p>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        searchButton.addEventListener('click', function() {
            ...
            if (!billNumber || isNaN(billNumber) || billNumber <= 0) {
        ...
        <?php if ($guest): ?>
            ...
            // Safely set issued_by
            issuedBySpan.textContent = <?php echo json_encode($guest['issued_by'] ?? 'Admin'); ?>;
        <?php else: ?>
            printButton.disabled = true;
        <?php endif; ?>
    });
