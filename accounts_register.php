...
$lock_result = $conn->query("SELECT is_locked FROM accountsregistration_access LIMIT 1");
        ...
        if (!$conn) {
            ...
            if ($stmt === false) {
                ...
                if ($result->num_rows > 0) {
                    ...
                    // Insert new user
                    $insert = $conn->prepare("INSERT INTO accounts_users (username, password) VALUES (?, ?)");
                    if ($insert === false) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['unlock_registration'])) {
    ...
    if (!$conn) {
        ...
        $admin_username = 'AccountsAdmin';
        $admin_password = '123456';
        
        if ($username === $admin_username && $password === $admin_password) {
            $update = $conn->prepare("UPDATE accountsregistration_access SET is_locked = FALSE");
            if ($update === false) {
...
<body>
    ...
    <div class="login-container">
        ...
        <form action="accounts_register.php" method="POST">
        ...
        <div class="register-link">
            ...
            <?php else: ?>
                <p>Already have an account? <a href="accounts_login.php">Login here</a></p>
            <?php endif; ?>
        </div>
    </div>
