...
try {
        ...
        case 'monthly':
            ...
            $stmt = $pdo->prepare("
                SELECT 
                    ...
                    SUM(p.stock * p.unit_price) as daily_total
                FROM purchased_items_backup p 
        ...
        case 'yearly':
            ...
            $stmt = $pdo->prepare("
                SELECT 
                    ...
                    MAX(p.expiry_date) as expiry_date,
                    SUM(p.stock) as total_stock,
                    SUM(p.stock * p.unit_price) as total_amount,
    ...
    foreach($report_data as $row) {
        if($report_type == 'daily') {
            $total_amount += $row['stock'] * $row['unit_price'];
        ...
        <form method="GET" action="">
            <div class="filters">
                ...
                <div class="filter-group">
                    ...
                    <input type="month" name="selected_month" value="<?= $selected_month ?>" onchange="this.form.submit()">
                </div>
                <?php else: ?>
                <div class="filter-group">
                ...
                <tbody>
                    ...
                    <tr>
                        <?php if($report_type == 'daily'): ?>
                            ...
                            <td>Rs.<?= number_format($row['stock'] * $row['unit_price'], 2) ?></td>
                        <?php elseif($report_type == 'monthly'): ?>
