...
try {
        ...
        try {
            ...
            try {
                ...
                foreach ($items as $item) {
                    ...
                    // Validate unit type (ensure it matches base unit or allowed conversion)
                ...
                // Insert aggregated items into order_sheet
                $stmt = $conn->prepare("INSERT INTO order_sheet (item_id, requested_qty, requested_unit, status, request_date, order_sheet_no, responsible_id, function_type, function_date, day_night) 
                                      VALUES (?, ?, ?, 'pending', NOW(), ?, ?, ?, ?, ?)");
                foreach ($aggregated as $item_id => $data) {
                    // Use NULL for day_night if empty
                    $day_night_value = empty($day_night) ? null : $day_night;
                    $stmt->execute([$item_id, $data['converted_qty'], $data['base_unit'], $order_sheet_no, $responsible_id, $function_type, $function_date, $day_night_value]);
                    error_log("Inserted aggregated item: ID={$item_id}, Converted Qty={$data['converted_qty']} {$data['base_unit']}, Order=$order_sheet_no, Function=$function_type, Date=$function_date, Day/Night=$day_night_value");
                }

                // Update remaining_quantity to 0 for SKY Buffer Stock Refill items
                if ($function_type === 'Main/K Buffer Stock Refill') {
                    $update_stmt = $conn->prepare("UPDATE kitchen_buffer SET remaining_quantity = 0 WHERE item_id = ?");
                    foreach ($aggregated as $item_id => $data) {
                        $update_stmt->execute([$item_id]);
                        error_log("Updated remaining_quantity to 0 for item_id: {$item_id}");
                    }
                }

                // Prepare print data with aggregated quantities and base units
                $print_items = [];
                foreach ($aggregated as $item_id => $data) {
                    $print_items[] = [
    ...
    // Fetch buffer stock items for Main/K Buffer Stock Refill, excluding items with remaining_quantity = 0
    $buffer_items = [];
    $stmt = $conn->prepare("
    ...
    foreach ($pending_requests as $request) {
        ...
        if (!isset($grouped_requests[$order_sheet_no])) {
            $grouped_requests[$order_sheet_no] = [
                ...
                'responsible_name' => $request['responsible_name'] ?? 'N/A', // Use responsible_name from query
...
<!-- ... (HTML and JavaScript as provided in your code) ... -->
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <select id="function_type" name="function_type" required class="block w-full max-w-md p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 sm:text-sm transition duration-200">
                        ...
                        <option value="Main/K Buffer Stock Refill">Main/K Buffer Stock Refill</option>
                    </select>
                </div>
                <div>
    ...
    <script>
        ...
        // Units that allow selection (ONLY kg and l)
        ...
        searchInput.addEventListener('input', function() {
            ...
            if (functionTypeSelect.value === 'Main/K Buffer Stock Refill') return;
